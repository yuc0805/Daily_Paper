...
2026-08 | ReasonCast (2608.01875) | fine-tuned LLM emits a reasoning chain and a numeric forecast in one pass, with the ReasonTS-Bench benchmark
2026-08 | WIRED (2608.12998) | adaptive mixture marginals recombined by a copula, keeping cross-series dependence out of the marginal objective
2026-08 | RouteTS (2608.14682) | amplitude-spectrum routing sends dominant frequencies to a complex linear predictor and the residual back to a time-domain MLP
2026-08 | ECG Context Length (2608.12695) | controlled ablation on single-lead ECG; minute-scale context and continuous patches beat short windows and quantised tokens
2026-09 | P2E-VQ (2608.14656) | quantises PPG patches to discrete tokens that index a memory bank of ECG-linked representations, so no ECG is generated at inference

2026-09 | Recurrent-State Write-Back (2609.04490) | 4-bit storage of a trained recurrent state raises error 70x to 300x; error feedback recovers it without retraining

2026-09 | Memory in Deep Time-Series Models (2609.06006) | survey that re-reads the field along one axis, from internal memory held in parameters and fixed-size states to external memory that is addressable, retrievable and agent-maintained
...
[2026] 2608.01875 — ReasonCast: Towards Explainable Time Series Forecasting with Reasoning. [https://arxiv.org/abs/2608.01875](https://arxiv.org/abs/2608.01875). external.
[2026] 2608.12998 — WIRED: Weighted Adaptive Prediction with Structured Dependence for Probabilistic Multiseries Forecasting. [https://arxiv.org/abs/2608.12998](https://arxiv.org/abs/2608.12998). external.
[2026] 2608.14682 — RouteTS: Frequency-Time Routing for Time Series Forecasting. [https://arxiv.org/abs/2608.14682](https://arxiv.org/abs/2608.14682). external.
[2026] 2608.12695 — The Impact of Temporal Context Length and Encoding Strategies on Self-Supervised ECG Representation Learning. [https://arxiv.org/abs/2608.12695](https://arxiv.org/abs/2608.12695). external.
[2026] 2608.14656 — P2E-VQ: ECG-linked representation augmentation for PPG via discrete patch retrieval. [https://arxiv.org/abs/2608.14656](https://arxiv.org/abs/2608.14656). external.

[2026] 2609.04490 — When Quantization Breaks Memory: Recurrent-State Write-Back in Low-Precision Temporal Inference. [https://arxiv.org/abs/2609.04490](https://arxiv.org/abs/2609.04490). external.
...
2026-09-01 | 2608.14656 added | P2E-VQ drops ECG reconstruction from PPG and does retrieval instead, cutting PPG into patches, quantising each patch to a discrete token, and using that token to index a memory bank of ECG-linked representations built only from training data, so inference needs PPG alone; across five public datasets and six downstream tasks spanning clinical endpoints and affective state it beats pretrained baselines under one frozen-feature linear-probing protocol; where Sensor2Text (ELYUE3NF) mapped a sensor stream into a richer modality and treated the generated target as the output, this paper keeps the cross-modal supervision but throws the generated target away, because the mapping is ill-posed and waveform fidelity does not track downstream accuracy, and where SSL for HAR with 700K person-days (RTMH75VW) improved the encoder by pretraining on more unlabelled signal, this paper leaves the encoder's input distribution alone and spends an ECG-paired dataset on a retrieval index instead, which is a different use of the same data; Foundation Models for Biosignals (2XWEG7AF) sets the pretrain-then-probe frame both share and the contribution is measured inside it rather than against it; the imported method is discrete tokenisation plus retrieval augmentation from language modelling and it is now closed off in the PPG-to-ECG direction, but the reusable part is the reframing rather than the architecture, namely that when a cross-modal mapping is ill-posed one should retrieve the paired representation instead of generating the paired signal, which carries to IMU-to-video, PPG-to-blood-pressure, and other wearable pairings with under-determined reconstruction targets; Tier B

2026-08-29 | 2608.12695 added | a controlled ablation on Icentia11k single-lead ECG varies only the input horizon (16 seconds, 1 minute, 5 minutes, 10 minutes) and the front end (continuous convolutional patch embeddings against fixed vector-quantised tokens) while holding the Transformer backbone and training protocol fixed, scoring representations by downstream abnormal rhythm detection and by patient-level retrieval across sessions, with the 5- and 10-minute models strongest and continuous patches ahead of discretised tokens at every horizon; where Chronos (72DFULQQ) made quantisation the default front end so that a language-model objective would apply, and PatchTST (YY67LF3R) argued for patching on efficiency and forecasting grounds, this paper measures what the conversion costs and finds it removes waveform detail that cross-session patient retrieval depends on, which is a stricter test than forecasting error; HeAR (3LA8GNCU) and most physiological encoders inherit a window of seconds from clinical reading practice, and the retrieval result says the individual-specific structure sits at the minute scale instead; the imported method is masked-autoencoder pretraining from vision and is closed off, leaving open whether the context finding holds for slower physiology such as PPG-derived heart rate variability or accelerometry; EMBC 2026; Tier B

2026-08-19 | 2608.14682 added | RouteTS partitions the amplitude spectrum of an input window and sends each part to the domain where it is best represented, keeping dominant high-amplitude frequencies in the frequency domain under a complex-valued linear predictor that preserves phase and avoids periodic misalignment over long horizons, and inverting residual spectral energy back to the time domain for a lightweight MLP so transient spikes are never smoothed by a Fourier basis, with the routing decision read from the data rather than a learned gate; where FEDformer sampled Fourier modes at random, DLinear argued a single time-domain linear layer suffices, and TimesNet extracted periodicity through a 2D reshape, RouteTS keeps the frequency domain only where it is the correct basis and separates the periodic and transient jobs, because a fixed mode subset cannot distinguish a component that is low-amplitude through unimportance from one whose energy is a spread-out transient; amplitude routing is native signal processing rather than an import from computer vision or language, so the idea is still upstream of this area and cheap to attach as a front end to an existing encoder; Tier A

2026-08-19 | 2608.12998 added | WIRED forecasts many related series jointly while keeping marginal distributions and cross-series dependence structurally separate, drawing marginals from a library of simple predictive distributions combined by CRPS-based adaptive mixture weights and reconstructing dependence afterwards through a Gaussian or Student t copula used for cross-series simulation, evaluated on four synthetic data-generating processes at three horizons with 30 replicates, nine ablations, and a rolling-origin study on EuStockMarkets; where Forecast Collapse (2608.14106) diagnosed that per-series objectives leave cross-series structure unidentified and Cross Variable Loss (2608.05742) patched the training objective with a graph penalty over residual edges, WIRED removes the coupling altogether, because dependence carried by an explicit copula cannot be destroyed by any property of the marginal objective, at the cost of imposing a parametric dependence family; the authors describe the contribution as architectural and diagnostic rather than a new state of the art, and the transfer direction is unusual in that classical statistics is being applied to a deep-learning failure mode; Tier B

2026-08-19 | 2608.01875 added | ReasonCast fine-tunes an LLM backbone to emit a reasoning chain and a numeric forecast together in one autoregressive pass rather than routing understanding and generation down separate task-specific paths, and releases ReasonTS-Bench, which decomposes time series into five fundamental patterns and scores forecast error alongside three reasoning metrics averaged over those primitives; where ChatTS aligned a series so an LLM could describe it and TS-Agent made reasoning verifiable by pushing every numeric claim into a tool call recorded in an evidence log, ReasonCast puts both capabilities inside a single forward pass and turns the chain into a supervised training target, because a trained chain can be scored on a fixed benchmark whereas a prompted or tool-mediated chain cannot be compared across backbones; the imported method is supervised reasoning-chain fine-tuning from language modelling and should be treated as ported and low-yield, leaving the bio-signal reasoning benchmark as the part still unbuilt; Tier B
