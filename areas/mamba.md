...
2026-09 | Kalman Delta Networks (2609.07816) | writes the fixed-size memory of linear attention as a linear-Gaussian state-space model and uses the Kalman gain as the write rule, with diagonal and isotropic approximations that keep the associative scan
...
[2026] 2609.07816 — Kalman Delta Networks: Uncertainty-aware Associative Memory. [https://arxiv.org/abs/2609.07816](https://arxiv.org/abs/2609.07816). external.

### Recent Activity

2026-09-09 | 2609.07816 added | Mamba-2 (JPKDWV2Q) showed that a linear-attention layer and a structured state-space model are the same recurrence and then spent its effort making that recurrence fast, while this paper asks what the recurrence should be estimating and answers with the Kalman filter, so the transition carries the memory state and its covariance together and each residual write is weighted by accumulated evidence and observation reliability rather than by the current token; where HiPPO (5YQ4IKMA) fixed the compression measure in advance and derived a static transition from it, effective write strength here moves with the tracked covariance, and the delta rule is recovered as the special case that substitutes a token-wise isotropic surrogate and drops covariance tracking, while the unbounded key-value cache of Attention Is All You Need (PHB9VRVM) never faced the overwrite question at all; the exact Riccati recursion is sequential and therefore unusable in a GPU-parallel scan, so the two approximations that preserve associativity, Diagonal KDN and Isotropic KDN, are what make the idea trainable, and both improve perplexity and mean downstream accuracy over linear-attention baselines in controlled pretraining at 750M and 1.3B parameters; the import runs from signal processing into language modeling rather than the reverse, and no wearable paper in community 5 applies uncertainty-tracked associative memory to physiological streams, where heteroscedastic observation noise from motion artifact on PPG and ECG would make the modeling assumption true; Tier A
