...
2026-09 | ASPIRE (2608.31111) | agent receives only a vague capability goal and must build its own data, metric and stopping rule; gains are sparse and do not transfer to the hidden set

2026-09 | Iris (2609.04304) | search agent trained by alternating supervised fine-tuning and live-search RL, on questions reverse-built from hyperlink structure

2026-09 | NeoHorse-1 (2609.08183) | serving traffic becomes training data, with the router capability estimate reused as a curriculum label; macro-average rises from 58.94 to 64.87 at 4B and 65.60 to 69.04 at 9B

2026-09 | Co-Evolving Harnesses and Models (2609.09134) | fine-tuning a weak model on expert trajectories under a harness evolved for that weak model regresses on all seven enterprise tasks; minimal on-policy expert correction recovers the gains
...
[2026] 2608.31111 — Aspire: Can Models Self-Evolve from Vague Goals?. [https://arxiv.org/abs/2608.31111](https://arxiv.org/abs/2608.31111). external.

[2026] 2609.04304 — Iris: Climbing to the Search Frontier. [https://arxiv.org/abs/2609.04304](https://arxiv.org/abs/2609.04304). external.

[2026] 2609.08183 — NeoHorse-1: Towards Recursive Self-Improvement via Agentic Post-Training with Routing Harness. [https://arxiv.org/abs/2609.08183](https://arxiv.org/abs/2609.08183). external.

[2026] 2609.09134 — Co-Evolving Harnesses and Models: On-Policy Correction Helps Weaker Models Catch Up Where Imitation Fails. [https://arxiv.org/abs/2609.09134](https://arxiv.org/abs/2609.09134). external.

### Recent Activity

2026-09-10 | 2609.09134 added | This paper evolves an agent harness, meaning the system prompt, tool set, execution hooks and context management around a model, for a weak model and then fine-tunes that weak model on a stronger expert's trajectories under the same harness; across seven enterprise agent tasks the combination regresses on all seven, by 4 to 30 points for Qwen3-Coder and Gemma 4, even though the identical fine-tuning helps when the harness has not been evolved. The diagnosis is a fit problem: imitation transfers knowledge and raises scaffold usage but installs the expert's planning strategy in a model that cannot execute it, and that strategy no longer matches a harness shaped around the weak model's native planning. The fix is on-policy expert correction, automated by a meta-level agent that locates the failing turn in the weak model's own rollout and has the expert rewrite only that turn, preserving the planning style and recovering gains from both sides. Where ADaPT (J8DYBKW2) matched decomposition depth to executor competence inside a single run, the same mismatch appears here across a training boundary, because fine-tuning silently overwrites the executor's planning style while the harness stays fixed; where LATS (77ERE7HA) and the scaffold line treated harness improvement and weight improvement as independent additive gains, this shows they interact negatively once the harness has been evolved against the weak model, a result the additive assumption cannot produce; and where NeoHorse-1 (2609.08183) trained on harness-logged trajectories to good effect, this isolates the case that reverses it, since those trajectories came from the model's own serving traffic while these come from a stronger expert, and on-policy versus off-policy provenance is what flips the sign. The transferable finding is that a scaffold tuned to a model becomes a liability once that model is retrained, which generalizes past agents to any pipeline where a hand-tuned preprocessing or prompting layer sits in front of a model that later gets fine-tuned, and it is a clean negative result with a stated mechanism and a fix, the shape of paper worth imitating; two papers in two days, one from RL post-training and one from agent scaffolding, now converge on the point that supervision has to be delivered on the student's own rollouts rather than the teacher's; Tier B

2026-09-09 | 2609.08183 added | LATS (77ERE7HA) searched over agent trajectories at inference time and discarded the search once an answer was produced, while NeoHorse retains the trace and feeds it back as supervision, because the expensive part of an agent rollout is the harness context; the capability demand that ADaPT (J8DYBKW2) computed on demand to decide how much planning a step needed is here predicted before the turn and used twice, first to route to a service tier and then to place the resulting example in a three-stage curriculum, and unlike the expert routing of DeepSeek-V3 (2JCKA7GI), which leaves the decision as internal activations inside one forward pass, routing across a pool of separate models leaves a capability estimate that survives outside the model and can label data; records are admitted through structural validation, six-dimensional semantic evaluation and subscene-level labeling, and capability-guided allocation turns evaluation feedback into the next training mixture; across eleven benchmarks the macro-average rises from 58.94 to 64.87 at 4B and from 65.60 to 69.04 at 9B, roughly six and three and a half points, in a setting where a self-selected data pipeline makes contamination hard to rule out and where one round of a loop is evidence about one round only; the transferable structure is using a routing or triage decision as a free label, which maps onto deployed sensing systems that already decide how much computation a window deserves and then discard that decision; Tier B

2026-09-07 | 2609.04304 added | Iris releases search agents at 35B-A3B and 397B-A17B together with the data pipeline and training recipe, building training questions backward from the hyperlink structure of a web corpus so that multi-hop chains are authored over an entity graph distilled from a seed page and its out-links, every non-answer entity is rewritten as a descriptive reference that cannot be resolved by string matching, and only questions a reference model fails closed-book but solves with the supporting evidence are admitted; supervised fine-tuning and reinforcement learning against live search alternate in a loop that feeds the hardest solved and most efficient rollouts of each round back into the next supervised pass, reaching 82.2/84.8/86.9/52.3 and 88.6/85.1/92.9/56.4 on BrowseComp, BrowseComp-ZH, DeepSearchQA and HLE from a single ReAct agent with no sub-agents and no test-time verification; where LATS (77ERE7HA) bought capability at inference time by expanding a search tree over candidate trajectories, Iris moves that capability into the weights and then runs a single pass, because the training loop can afford the expensive exploration once while deployment cannot afford it per query, and where DeepSeek-R1 (Z5IWHZAE) ran outcome-reward reinforcement learning against a verifier on a fixed answer set, Iris runs it against live search, which makes the environment nonstationary and forces the serving engineering the paper spends most of its length on, namely reward judge and observation summarizer inside the training cluster and over-long rollouts interrupted at the request level and resumed from a committed prefix, while DeepEyesV2 (4XXXYXS9) supplies the single-policy interleaving of tool calls and reasoning that Iris scales; the measurement is the part worth taking, since every benchmark is reported with and without inference-time context management and the gap that management produces is larger than most reported differences between competing systems, which is a reason to treat single-number agent comparisons in reviews with suspicion, and the reverse-construction recipe is portable because the same procedure would run over a sensor-record graph rather than a hyperlink graph; Tier B

2026-09-04 | 2608.31111 added | ASPIRE gives an agent only a natural-language capability goal such as improve at research, with no downstream task and no metric, so the agent must interpret the goal, choose training data and update method, build its own training and validation signal, and decide when to stop, with evaluation held out on 520 expert-authored items across six goals and both weight-level and harness-level self-evolution running in one interactive environment; agents complete the loops, but weight-level gains are sparse and unstable, the best evolved harness stays below the engineered Qwen-Agent reference, agents often train on mismatched data, and continued search can erase earlier gains; where Rethinking Memory Mechanisms of Foundation Agents (BDY3HUCV) and the Agent AI Survey (Z9WZPMNU) both assume the objective is given and ask how the agent should be built, ASPIRE removes the objective and asks whether the agent can construct one, because specifying the metric is the part of research work that humans currently do and that self-evolution claims implicitly cover, and where Repo-To-Skill (2609.02749) improved a fixed agent by mining verified skills from an external corpus and checking them against known tasks, ASPIRE denies the agent both the corpus and the verification target, with the reported failure landing precisely at that seam, since agents trust narrow self-evaluations and local gains do not transfer to the hidden set; read together, the two papers from consecutive days bracket the same question, one showing what external verified knowledge buys and the other what its absence costs, and the hidden-evaluation design is a cheap test of whether any self-improving system has learned the goal or only a proxy, which carries directly to a self-tuning sensing pipeline that picks its own validation split; Tier B
