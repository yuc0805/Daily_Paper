...
2026-09 | RISE (2609.05295) | extrapolates the displacement between a checkpoint and a trailing anchor into a synthetic teacher, making outcome rewards dense

2026-09 | One Symptom, Three Levers (2608.25936) | review of on-policy self-distillation; names reasoning-path narrowing as the shared failure mode and sorts the field by three levers
2026-09 | FlowBalance (2609.03241) | trajectory-level normalised distribution that combines a sparse terminal verifier with a dense self-guidance score; avoids response-length collapse

2026-09 | OPRD (2609.08798) | amplifies the component of the student verifier-driven gradient lying along the weak teacher policy shift, so the teacher accelerates the student without becoming a target

2026-09 | Revisiting Complete Reasoning Traces (2609.07103) | reports that supervised fine-tuning on full reasoning trajectories gives limited benefit over heavily truncated ones; intermediate tokens contribute little
...
[2026] 2609.05295 — RISE: Recursive Improvement via Self-Extrapolating Policy Distillation. [https://arxiv.org/abs/2609.05295](https://arxiv.org/abs/2609.05295). external.

[2026] 2608.25936 — One Symptom, Three Levers: A Critical Review of On-Policy Self-Distillation. [https://arxiv.org/abs/2608.25936](https://arxiv.org/abs/2608.25936). external.
[2026] 2609.03241 — FlowBalance: Verifier-Grounded Self-Improvement from On-Policy Reasoning Experience. [https://arxiv.org/abs/2609.03241](https://arxiv.org/abs/2609.03241). external.

[2026] 2609.08798 — Eliciting Weak-to-Strong Generalization with On-Policy Reverse Distillation. [https://arxiv.org/abs/2609.08798](https://arxiv.org/abs/2609.08798). external.

[2026] 2609.07103 — Revisiting Complete Reasoning Traces for Post-Training. [https://arxiv.org/abs/2609.07103](https://arxiv.org/abs/2609.07103). external.

### Recent Activity

2026-09-10 | 2609.07103 added | This paper asks whether supervised fine-tuning on complete reasoning trajectories is what makes post-training work and reports that it largely is not: a pilot study finds full trajectories give only limited benefit over partial ones and that partial trajectories remain effective under heavy truncation, while attention-based analysis and controlled token-removal studies indicate intermediate tokens contribute little to final reasoning quality, which the authors read as a model inferring coherent intermediate steps from its own internal knowledge once given the endpoints, making much of the middle of a long chain redundant supervision rather than necessary supervision. Where Chain-of-Thought Prompting (HBLPTRMY) showed that producing intermediate steps at inference time helps, this separates that from the claim that consuming them at training time helps and finds the second much weaker than assumed; where DeepSeek-R1 (Z5IWHZAE) established long chains as the shape of a strong reasoner, this argues the length is partly incidental to what is learned, since truncated supervision reaches comparable quality; and where CODI (FFWLYL3J) compressed the chain into latent form and still had to represent it, this reports that much of the chain can simply be deleted from the training data, a stronger and cheaper claim if it holds. The instrument is attention analysis attached to a controlled intervention, and the intervention is the part that carries weight. The consequence is first a cost argument: if heavily truncated trajectories match full ones, the data budget for reasoning post-training drops by a large factor and long-trace collection stops being the bottleneck outside a frontier lab. The open transfer question is whether this holds when intermediate steps are numerical operator calls rather than natural-language sentences, since in a time-series agent the middle of the trace is where evidence is gathered and cannot be inferred from internal knowledge the way an arithmetic step can; that contrast is testable and small. It is also a caution against reading trace length as evidence of reasoning quality in any benchmark built here; Tier B

2026-09-09 | 2609.08798 added | conventional distillation makes the teacher output distribution the optimization target and therefore transfers the teacher capacity ceiling along with its knowledge, so OPRD never makes the teacher a target at all: it measures the weak teacher policy shift relative to that teacher own reference policy, evaluated on the student rollouts rather than the teacher rollouts, and amplifies the component of the student verifier-driven policy gradient lying along that direction; where DeepSeek-R1 (Z5IWHZAE) obtained reasoning purely from verifier reward and paid for it in sample count, OPRD keeps the same verifier signal and spends a weak teacher to reach the same place in fewer student updates, and where PPO (KQTIJC4U) constrains the update isotropically to a trust region for stability, OPRD applies an anisotropic rescaling that deliberately stretches one direction and relies on the verifier-support restriction rather than a trust region to keep it safe, with the stationary-point argument of Policy Gradient Methods (DAGWCZ67) carrying the claim that only the path changes and not the optimum; across successive model transfer and multi-teacher consolidation it reaches competing final performance earlier and then passes it, and response-style analysis finds the students resemble verifier-only RL models more than they resemble their teachers, which is the evidence that guidance accelerated rather than redirected them; the reusable idea is guidance as a direction rather than a target, and its dependency is a verifier, so what plays that role outside math and code is the open question worth carrying into time-series work; Tier B

2026-09-08 | 2609.03241 added | FlowBalance learns a normalised distribution over complete responses rather than a per-token objective, and combines two supervision sources that fail in opposite directions, a terminal verifier that gives reliable but sparse signal and a frozen training-time copy of the same policy, given privileged context, that gives dense token-level log-probability gains aggregated into one trajectory-level self-guidance score; on mathematical reasoning it improves average accuracy over FlowRL on both Qwen3-4B and Qwen3-8B, trains faster and more stably, avoids the response-length collapse seen in direct on-policy self-distillation, and keeps higher correct-strategy diversity in a controlled AIME24 diagnostic; where DeepSeek-R1 (Z5IWHZAE) accepted sparse outcome reward and paid for it in sample efficiency, FlowBalance keeps that reward as the ground truth and adds a dense signal drawn from the model itself, so the two supervision sources are combined rather than substituted, where Chain-of-Thought Prompting (HBLPTRMY) treated the reasoning trace as a prompting artefact, this paper treats the whole trajectory as the unit of the learned distribution, which is what allows the length-collapse failure mode to be diagnosed as a distributional effect rather than a decoding one, and against Latent-GRPO (U4ZPM5DN) the change is not the optimiser but what the advantage is computed over; the self-guidance score is deliberately aggregated to trajectory level rather than applied per token, because the paper's diagnosis is that per-token privileged guidance is what narrows the set of reasoning paths; this is the concrete method in a cluster that has now produced four papers in two days and the one that reports the diversity diagnostic rather than only the benchmark average, so where a self-generated teacher is used the AIME24 correct-strategy-diversity protocol is the measurement worth copying, because average accuracy hides collapse until it is too late to attribute; Tier B

2026-09-08 | 2608.25936 added | a review with no new experiments that defines on-policy self-distillation as the case where the teacher is the model itself conditioned on privileged information the student will not have at test time, such as a reference solution, a plan or environment feedback, so the teacher is no stronger than the student and only better informed; it argues that one failure mode, the progressive narrowing of the set of reasoning paths the model can produce, dominates the subfield, and sorts the literature by three levers that govern it, how tokens are weighted, what the teacher is shown, and how the guidance changes over training, with scope restricted to mathematical reasoning; where DeepSeek-R1 (Z5IWHZAE) answered the question of how to obtain reasoning at all, this review takes that answer as settled and asks what the cheaper substitutes cost, which is a maturity signal about the subfield rather than a new capability, and where RISE (2609.05295) proposed one specific self-teacher and reported gains on mathematics, code and agentic tasks, this review names the failure mode that any such self-teacher inherits and separates the settled claims from the disputed ones, so RISE reads as one setting of the three levers rather than as a standalone method; it also makes explicit a bridge already present in the graph, since DeepSeek-R1 and Chain-of-Thought are linked as reasoning through reinforcement learning against reasoning through prompting, and self-distillation sits between them, using a prompted teacher to supply the dense signal that reinforcement learning does not; reading it before the individual method papers saves time on the whole cluster and the vocabulary it fixes is what makes FlowBalance, TGOPD and RISE comparable at all, though it is short on new content by design and the two-page test applies with full force; Tier B

2026-09-07 | 2609.05295 added | RISE builds a synthetic teacher out of the model's own reinforcement learning trajectory, taking the displacement between the current checkpoint and a trailing anchor checkpoint in parameter space or in output logit space and extrapolating along it, which converts a sparse outcome-induced parameter update into a dense token-level target with no external model and no privileged conditioning; outcome rewards from RLVR ground the extrapolation toward correct reasoning while the extrapolated teacher refines token-level decisions, and because the teacher is regenerated each iteration as the student improves, distillation becomes a recursive loop rather than a one-shot compression, with reported gains over RLVR-only training and on-policy self-distillation across mathematical reasoning, multi-domain STEM, code generation and multi-turn agentic tasks; where DeepSeek-R1 (Z5IWHZAE) turned a sparse outcome reward into a parameter update and stopped there, RISE reads that same update as a direction and steps further along it, because the displacement between two checkpoints already encodes what the reward was pushing toward at every token, which converts one bit of outcome signal per rollout into supervision on every position and closes the density gap between RLVR and distillation, while Chain-of-Thought (HBLPTRMY) fixes the token-level trace that both the reward and the distillation target act on; against Rethinking On-Policy Distillation II (2609.04172), read two days earlier, the two agree on the diagnosis that on-policy distillation is algorithm-starved rather than data-starved and differ on the fix, since that paper removed the data and this one replaces the teacher, so the reasoning-via-RL against reasoning-via-prompting bridge now runs toward treating optimizer trajectories rather than models as the source of supervision; checkpoint extrapolation is a mechanism rather than a task-specific trick and needs only a training trajectory and a trailing anchor, so it transfers to any setting where the reward is sparse and per-step targets are missing, which describes most sequence-labelling and forecasting objectives on physiological data where only an episode-level outcome is known, and the failure mode to check is whether extrapolation amplifies a systematic error in the trajectory rather than the intended direction; Tier B
